...
// @desc    Update admin status for multiple orders
// @route   PUT /api/v1/orders/admin-status
// @access  Protected/Admin-Superadmin
exports.updateAdminStatusBulk = asyncHandler(async (req, res, next) => {
  const { orderIds, status } = req.body;
  const allowed = ['completed', 'waiting', 'out_of_stock'];
  if (!Array.isArray(orderIds) || orderIds.length === 0 || !allowed.includes(status)) {
...
// @access  Protected/User-Admin-Superadmin
...
// @desc    Mark order as invoiced
// @route   PUT /api/v1/orders/:id/invoice
// @access  Protected/Admin-Superadmin
exports.markOrderInvoiced = asyncHandler(async (req, res, next) => {
  const order = await Order.findById(req.params.id);
  if (!order) {
    return next(new ApiError(`No order found for ID ${req.params.id}`, 404));
  }
  ...
  order.invoiceNumber = req.body.invoiceNumber || String(order._id).slice(-8).toUpperCase();
